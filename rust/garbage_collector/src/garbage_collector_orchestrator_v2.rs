...
impl GarbageCollectorOrchestrator {
    ...
    async fn start_delete_unused_logs_operator(
        ...
        for (collection_id, versions) in &versions_to_delete.versions {
            ...
            tracing::debug!("VERSION FILE IS: {:?}", version_file);
    ...
    async fn try_start_delete_versions_at_sysdb_operator(
        ...
        for (collection_id, versions) in versions_to_delete {
            ...
            match self.database_name.as_ref() {
                Some(db_name) => tracing::info!("HAVE DB NAME {}", db_name.clone().into_string()),
                None => tracing::info!("NO DB NAME???"),
            };
            ...
            for collection_id in ordered_soft_deleted_to_hard_delete_collections {
                ...
                self.sysdb_client
                    .finish_collection_deletion(
                        ...
                        // Note(tanujnay112): This is due to the fact that there was a bug
                        // by which we were passing an empty string here in the single region case.
                        // This was due to the fact that an empty database name was getting persisted
                        // in version files from which we derived this field.
                        database_name
                            .map(|db| db.into_string())
                            .unwrap_or("".to_string()),
                        collection_id,
                    )
                    .await?;
